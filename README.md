...
| 🇺🇿 **Mock exams (CEFR Multilevel)** | **Speaking** — 1.1 (3 questions, 30 s), 1.2 (two pictures described in words, 3 questions), 2 (topic with 3 guiding questions, 1 min prep + 2 min), 3 (statement with a for/against table, 1 min prep + 2 min); **Writing** — one situation email, 1.1 informal letter (50–70 words), 1.2 formal letter (120–150), 2 online discussion post (180–200), 60 minutes, live word-range check, draft survives a reload. Score **0–75 and level with the official cut-offs** (uzbmb.uz: C1 65–75, B2 51–64, B1 38–50). Part format follows current preparation materials (new format since 2024 H2); Speaking parts are marked 5/5/5/6 → 75; Writing task weights are not published, so we use 1 : 2 : 3 in proportion to length — stated on the result page. Listening and Reading follow once their format is confirmed. |
...
- **Tests:** 273 backend unit tests (xUnit) — scheduler, streak across time zones, card creation, grading, strict parsing, token hashing, XP/levels/badges, leaderboard ranking, level-aware prompts, vocabulary status, localized messages (every key in all three languages with matching placeholders), email-code rules (expiry, attempts, cooldown, hourly cap), email templates Google ID-token validation (forged signature, wrong audience/issuer, expiry, `alg: none`), AI quota, the Telegram bot logic, the system-status rules and IELTS band maths (official rounding, Task 2 weighting, word count, mock limits, question-bank format, objective grading and word limits, raw-score→band anchors, the test validator, answer-free client payloads, full-mock rounding, CEFR cut-offs and score conversion) — and 67 frontend tests with Vitest (charts, word tapping, vocabulary search, plurals).
