...
- **Tests:** 332 backend unit tests (xUnit) — scheduler, streak across time zones, card creation, grading, strict parsing, token hashing, XP/levels/badges, leaderboard ranking, level-aware prompts, vocabulary status, localized messages (every key in all three languages with matching placeholders), email-code rules (expiry, attempts, cooldown, hourly cap), email templates Google ID-token validation (forged signature, wrong audience/issuer, expiry, `alg: none`), AI quota, the Telegram bot logic, the system-status rules and IELTS band maths (official rounding, Task 2 weighting, word count, mock limits, question-bank format, objective grading and word limits, raw-score→band anchors, the test validator, answer-free client payloads, full-mock rounding, the official CEFR raw→75 table, row by row, teacher groups: join codes, assignment types, completion/lateness rules, score labels, what the teacher can see) — and 77 frontend tests with Vitest (charts, word tapping, vocabulary search, plurals).
