...
- **Tests:** 136 backend unit tests (xUnit) — scheduler, streak across time zones, card creation, grading, strict parsing, token hashing, XP/levels/badges, leaderboard ranking, level-aware prompts, vocabulary status, localized messages (every key in all three languages with matching placeholders), email-code rules (expiry, attempts, cooldown, hourly cap), email templates and Google ID-token validation (forged signature, wrong audience/issuer, expiry, `alg: none`) — and 34 frontend tests with Vitest (charts, word tapping, vocabulary search, plurals).
