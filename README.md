...
| 🇺🇿 **Mock exams (CEFR Multilevel)** | **Scoring follows the official assessment document** (Agency for Assessment of Knowledge and Skills, approved 16 March 2023, uzbmb.uz): the raw score (0–36, mean of examiners' marks) is converted to **0–75 with the official table**; Writing = Task 1 (12 points) + Task 2 (24 points); Speaking = one holistic score; levels C1 65–75, B2 51–64, B1 38–50. **Speaking** — 1.1 (3 questions), 1.2 (two pictures described in words, 3 questions), 2 (topic + 3 questions, 1 min prep + 2 min), 3 (statement with a for/against table, 1 + 2 min). **Writing** — one situation email, 1.1 informal letter (50–70 words) and 1.2 formal letter (120–150) together form Task 1, 2 is an online discussion post (180–200); 60 minutes, live word-range check, draft survives a reload. The task layout follows preparation materials for the new 2024 format; the score maths is official. Listening and Reading follow. |
...
- **Tests:** 298 backend unit tests (xUnit) — scheduler, streak across time zones, card creation, grading, strict parsing, token hashing, XP/levels/badges, leaderboard ranking, level-aware prompts, vocabulary status, localized messages (every key in all three languages with matching placeholders), email-code rules (expiry, attempts, cooldown, hourly cap), email templates Google ID-token validation (forged signature, wrong audience/issuer, expiry, `alg: none`), AI quota, the Telegram bot logic, the system-status rules and IELTS band maths (official rounding, Task 2 weighting, word count, mock limits, question-bank format, objective grading and word limits, raw-score→band anchors, the test validator, answer-free client payloads, full-mock rounding, the official CEFR raw→75 table, row by row) — and 67 frontend tests with Vitest (charts, word tapping, vocabulary search, plurals).
