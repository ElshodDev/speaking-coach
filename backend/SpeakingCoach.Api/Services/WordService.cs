...
{
    Task<WordExplanation> ExplainAsync(string word, string sentence, string level, string lang, CancellationToken ct = default);
}
...
public partial class GeminiWordService : IWordService
{
    ...
    /// Xato bo'lsa — null va xato kaliti (Texts'dagi, masalan "word.invalid").
    /// </summary>
    ...
    /// <summary>
    /// Gap bo'lsa — so'z aynan shu gapdagi ma'nosida; bo'lmasa (Lug'at
    /// bo'limida qo'lda yozilgan so'z) — eng ko'p ishlatiladigan ma'nosida.
    /// </summary>
    public static string BuildPrompt(string word, string sentence, string level, string lang = Texts.DefaultLang)
    {
        var context = string.IsNullOrWhiteSpace(sentence)
            ? "in its most common everyday meaning."
            : $"as it is used in this sentence:\n\"{sentence}\"";
        var (learner, translation) = lang switch
        {
            "ru" => ("a Russian-speaking", "short Russian translation"),
            "en" => ("an", "a simple English synonym or very short paraphrase"),
            _ => ("an Uzbek-speaking", "short Uzbek translation in Latin script (use the letters oʻ and gʻ)"),
        };
        return string.Format(PromptTemplate, word, context, LearnerLevel.Describe(level), learner, translation);
    }

    public async Task<WordExplanation> ExplainAsync(string word, string sentence, string level, string lang, CancellationToken ct = default)
    {
        var requestBody = new
        {
            contents = new[] { new { parts = new object[] { new { text = BuildPrompt(word, sentence, level, lang) } } } },
        ...
        if (string.IsNullOrWhiteSpace(result.Translation) || string.IsNullOrWhiteSpace(result.Word))
        {
