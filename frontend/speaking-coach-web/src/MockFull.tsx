...
export function MockFullStart({ go }: { go: (route: string) => void }) {
 ...
 */
export function MockFullStep({ variant, sessionId, step, go }: { variant: Variant; sessionId: string; step: number; go: (route: string) => void }) {
  const t = useT(mockObjMsg);
...
export function MockSessionView({ sessionId, go }: { sessionId: string; go: (route: string) => void }) {
  ...
  const variant = (Object.values(data.modules).find((m) => m.variant)?.variant as Variant | undefined) ?? 'academic';
  ...
  return (
    <>
      ...
      <div className="card" style={{ textAlign: 'center' }}>
        ...
        <div style={{ fontSize: '3rem', fontWeight: 800, lineHeight: 1.1 }} data-testid="session-overall">{formatBand(data.overall)}</div>
        {data.overall !== null ? (
          <div className="small">{tm.bandNames[Math.floor(data.overall)]}</div>
        ) : (
          <div className="muted small">{t.overallPending}</div>
        )}
      </div>
      <div className="card">
            ...
            {FULL_ORDER.map((m) => {
              ...
              return (
                <tr key={m}>
                  ...
                  <td style={{ fontSize: '1.2rem', fontWeight: 700 }}>{r ? formatBand(r.overall) : <span className="muted small">{t.notTaken}</span>}</td>
      ...
      <p className="muted tiny">{tm.disclaimer}</p>
    </>
  );
}
